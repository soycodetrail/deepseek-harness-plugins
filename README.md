...
>
> 🤖 本仓库内容由脚本自动同步自主站，**与网站保持实时同步**（source 数据变更 → GitHub Actions 推送即更新）。最近同步：2026-09-06 10:50:46
...
- [dsh-deploy-master](https://github.com/Olympianz/dsh-deploy-master) — A DSH deploy-assistant plugin: GitHub publish + Linear sync + npm publish + community announcement.  
  标签：插件 / 目录
- [dsh-ask-first](https://github.com/1052326311/dsh-ask-first) — Adaptive product discovery for DeepSeek Harness using native question dialogs.  
  标签：dsh-plugin / 核心引擎
...
- [dsh-goal](https://github.com/GooDAnDReaDY/dsh-goal) — Goal mode & autonomous execution plugin for DeepSeek Harness with sticky top banner  
  标签：dsh-plugin / 核心引擎
- [dsh-video-studio](https://github.com/lovstudio/dsh-video-studio) — DSH video editing workbench with Remotion, GSAP and pluggable ASR  
  标签：dsh-plugin / 核心引擎
- [dsh-qqbot](https://github.com/gcry13067381632-jpg/dsh-qqbot) — DSH 接入官方 QQ 机器人增：富媒体收发 · 表情包图库 · 多预设多开 · 定时唤醒 · 远程审批 · 延迟聚合回复 · 自主静默or回复  
  标签：dsh-plugin / 核心引擎
...
- [dsh-request-privacy](https://github.com/jhckevin/dsh-request-privacy) — Live request metadata minimization for DeepSeek Harness, covering the native DeepSeek provider with WebUI settings.  
  标签：dsh-plugin / 核心引擎
- [dsh-chat-fold](https://github.com/dusbin/dsh-chat-fold) — 折叠/展开AI回复的Think、Read、Edit 等操作内容，默认只展示重要信息  
  标签：dsh-plugin / 核心引擎
- [dsh-ticker-jp](https://github.com/MurasakiIzumi/dsh-ticker-jp) — 日股行情悬浮窗插件：DeepSeek Harness 右上角实时显示日经225 与 TOPIX ETF 报价（Yahoo Finance 数据源），支持自选任意代码与显示别名、休市自动降频、16 种界面语言。由 dsh-stock-ticker 改版而来。/Floating Japanese-stock ticker for DeepSeek Harness — real-time Nikkei 225 / TOPIX ETF quotes via Yahoo Finance, custom watchlist & aliases. Forked from dsh-stock-ticker.  
  标签：dsh-plugin / 核心引擎
- [dsh-plugin-insights](https://github.com/ice5kysl/dsh-plugin-insights) — DSH 插件生态质量评估数据层 · objective health scores (A-D, health-v1) for the whole DeepSeek Harness plugin ecosystem — 全量客观健康分/真伪判定/生态分析/开放数据  
  标签：dsh-plugin / 核心引擎
...
- [dsh-amphoreus](https://github.com/xi-kari/dsh-amphoreus) — 翁法罗斯 × DSH：黄金裔席位工作区与画布工作台（基于 DeepSeek Harness 构建，非官方）  
  标签：dsh-plugin / 核心引擎
- [dsh-model-groups](https://github.com/roclee2692/dsh-model-groups) — DeepSeek Harness grouped provider and manufacturer model selector  
  标签：dsh-plugin / 核心引擎
- [dsh-usage-insights](https://github.com/3361805598-gif/dsh-usage-insights) — Local-first personal usage insights for DeepSeek Harness  
  标签：dsh-plugin / 核心引擎
- [dsh-clawbot](https://github.com/cryjkd/dsh-clawbot) — 在 DeepSeek Harness（DSH）里接入微信官方 ilink 机器人网关：扫码绑定后，AI 能主动给自己发微信通知；每次回复后自动推送任务完成摘要；目标完成/阻断时自动通知；需要批准/选择时主动微信提醒。自带设置界面（设置 → ClawBot：扫码二维码、微信在线/断线状态、手动发送、三个自动通知开关、监听开关、解除绑定、自定义图标）。  底层走腾讯官方 ilink 网关（https://ilinkai.weixin.qq.com），无第三方破解、无自建中转，消息只发给绑定的账号自己。  
  标签：dsh-plugin / 核心引擎
- [DSH-P2M](https://github.com/GiantAxeint/DSH-P2M) — DSH 插件管家（dsh-p2m / A插件）：统一管理+冲突处理+按用时排序 | A plugin manager & conflict resolver for DeepSeek Harness (DSH/Cordis)  
  标签：dsh-plugin / 核心引擎
- [dsh-local-models](https://github.com/Vmarcelo49/dsh-local-models) — Use local models via Llama.cpp on your deepseek harness  
  标签：dsh-plugin / 核心引擎
- [dsh-message-rewrite](https://github.com/zouxiaoyang/dsh-message-rewrite) — Codex 风格消息原地编辑 DSH 插件：hover ✎ 编辑 → 同会话截断并重新生成回复（surface replacement）  
  标签：dsh-plugin / 核心引擎
...
- [owndsh](https://github.com/boe1900/owndsh) — The Self-Hosted Control Plane for DeepSeek-Harness  
  标签：dsh-plugin / 核心引擎
- [dsh-madrank](https://github.com/qomob/dsh-madrank) — AI usage dashboard + optional global ranking for DeepSeek Harness (DSH).  
  标签：dsh-plugin / 核心引擎
- [dsh-workbuddy-xdpool](https://github.com/aosi526/dsh-workbuddy-xdpool) — Merge every locally signed-in WorkBuddy account into DeepSeek Harness as one auto-failing-over model pool (multi-account rotation, live credits and model catalog).  
  标签：dsh-plugin / 核心引擎
- [dsh-company](https://github.com/TiferKing/dsh-company) — 不想当牛马了？来当老板！开插件公司，雇AI牛马，过赛博人生！  
  标签：dsh-plugin / 核心引擎
- [dsh-xiaomili](https://github.com/Olympianz/dsh-xiaomili) — 小米粒（dsh-xiaomili）—— 常驻 chatbox 右侧的金毛秘书插件：实时旁听对话、识别意图、提取关键词/概念、自动生成秘书简报，记忆可持久化。  
  标签：dsh-plugin / 核心引擎
- [dsh-background](https://github.com/retmon2333/dsh-background) — DeepSeek Harness（DSH）背景壁纸更换（单图 / 文件夹轮播）模式，可调透明、模糊、遮罩与樱花／雪花特效，侧栏与输入框也能透出壁纸  
  标签：dsh-plugin / 核心引擎
...
- [dsh-stream-market](https://github.com/Minglink/dsh-stream-market) — DeepSeek Harness 内置可视化插件市场 — 极速秒开、官方 dsh:// 联动、深度本地管理与原子化彻底卸载  
  标签：dsh-plugin / 核心引擎
- [dsh-sound-effects](https://github.com/fenghua00/dsh-sound-effects) — DeepSeek Harness 音效提示插件：任务完成或权限请求时播放提示音，可更换音效。  
  标签：dsh-plugin / 核心引擎
...
- [dsh-flywheel](https://github.com/zsxian11/dsh-flywheel) — 面向 DeepSeek Harness（DSH） 的「会话飞轮」插件 。在单会话隔离下，让一条龙长会话接近当前句只带工作集；用项目级产物图 + 倒排在会话之间传递短卡。  
  标签：dsh-plugin / 核心引擎
- [dsh-plugin-garmin](https://github.com/RayzPub/dsh-plugin-garmin) — dsh-plugin-garmin 是专为佳明表盘开发的 DeepSeek Harness (dsh) 插件。用户只需在聊天窗口中通过自然语言对话，即可完成针对 Garmin Fenix 7 系列（260×260、64 色 MIP 显示屏）的表盘设计、矢量仿真预览、资源合规校验、Monkey C 源码生成以及最终 .prg 安装二进制的编译打包  
  标签：dsh-plugin / 核心引擎
...
- [dsh-discussions-digest](https://github.com/kristol07/dsh-discussions-digest) — A small, evidence-first DeepSeek Harness plugin for recent activity in the official GitHub Discussions.  
  标签：dsh-plugin / 核心引擎
- [dsh-plugin-dev-assistant](https://github.com/CodeDice1024/dsh-plugin-dev-assistant) — DSH 插件开发助手：对话式引导，从零开始创建、测试、发布 DSH 插件  
  标签：dsh-plugin / 核心引擎
- [dsh-reasoning-options](https://github.com/Scorp1o117/dsh-reasoning-options) — DeepSeek Harness 插件：自动为 pi-ai 第三方模型添加推理强度（reasoning effort）选项 / Auto-add reasoning-effort picker to every pi-ai model in dsh  
  标签：dsh-plugin / 核心引擎
- [dsh-plugins](https://github.com/sanyamjain2014/dsh-plugins) — DeepSeek Harness (DSH) plugins by sanyamjain2014 — everything is a plugin.  
  标签：dsh-plugin / 核心引擎
- [dsh-openlist-sync](https://github.com/bg8lng/dsh-openlist-sync) — DeepSeek Harness 文件同步插件：交付文件自动上传 OpenList + 目录全读写工具 + 设置面板 + 按工作区布局  
  标签：dsh-plugin / 核心引擎
- [dsh-stall-sentinel](https://github.com/aokamoaki/dsh-stall-sentinel) — Lightweight stall watchdog for DeepSeek Harness - auto-wires subprocess spawn, reminds + forensics on stall, never kills.  
  标签：dsh-plugin / 核心引擎
- [dsh-plugins](https://github.com/klarkxy/dsh-plugins) — Small, independently installable plugins for DeepSeek Harness.  
  标签：dsh-plugin / 核心引擎
- [dsh-plugin-feihualing](https://github.com/zhaoxuejie/dsh-plugin-feihualing) — DeepSeek Harness 飞花令游戏插件：简易 / 古法严格双模式，按会话独立维护令字、得分、已用诗句与剩余提示次数  
  标签：dsh-plugin / 核心引擎
- [dsh-plugin-updater](https://github.com/hyqhyq3/dsh-plugin-updater) — Package-level plugin manager with update detection and a GitHub marketplace for DeepSeek Harness  
  标签：dsh-plugin / 核心引擎
- [dsh-moe-plugin](https://github.com/nataliwhite20534-droid/dsh-moe-plugin) — 10 preset moe character cards for DSH - freely combinable, editable, persistable. Real outputs in docs/usage-examples.md.  
  标签：dsh-plugin / 核心引擎
...
- [dsh-geek-ui](https://github.com/kaerf15/dsh-geek-ui) — DSH Geek UI — dsh web 极客增强插件集合（页头重排 / 极客侧栏 / 归档管理）  
  标签：dsh-plugin / 界面与体验
- [dsh-universal-worldbook](https://github.com/TritiumWang/dsh-universal-worldbook) — 酒馆风格的通用世界书，为最新用户消息附加一次性的提示词注入，无上下文污染。SillyTavern-style worldbook for general purpose. Generating per-request injections to user message without long-lasting context contamination.  
  标签：dsh-plugin / 界面与体验
- [dsh-session-manager](https://github.com/SunshineR04/dsh-session-manager) — DeepSeek Harness (DSH) plugin: manage archived sessions - list, restore, permanently delete; Settings page + red delete item in the session context menu  
  标签：dsh-plugin / 界面与体验
- [dsh-web-searxng](https://github.com/biyuhao/dsh-web-searxng) — Self-hosted SearXNG search backend for DSH — settings card, one-click connectivity tests, and a self-ranking community instance picker. No third-party search API needed.  
  标签：dsh-plugin / 界面与体验
- [dsh-clearview](https://github.com/Vinzelles/dsh-clearview) — A calmer reading view for DeepSeek Harness: native process details, live reasoning, and clean final answers.  
  标签：dsh-plugin / 界面与体验
...
- [DeepSeek-Harness-Desktop-Ultra](https://github.com/ijry/DeepSeek-Harness-Desktop-Ultra) — DeepSeek Harness桌面客户端封装版本基于tauri而不是electron体积更小  
  标签：dsh-plugin / 界面与体验
- [dsh-flykit](https://github.com/flykit-cc/dsh-flykit) — flykit for DeepSeek Harness — explorer with live file watch, agent terminals (Claude Code, Pi, Codex), DSH agent tools, a searchable model picker and a Claude-Code-style status line. Fly your agents.  
  标签：dsh-plugin / 界面与体验
- [llm-wiki-sidebar](https://github.com/ApeInCodeMountain/llm-wiki-sidebar) — LLM Wiki workbench for dsh-better-sidebar: browse and lint a Karpathy-style interlinked markdown wiki, open pages with the native markdown viewer, clickable [[wikilinks]] and provenance/relative .md links, and backlinks. Ships the llm-wiki agent skill.  
  标签：dsh-plugin / 界面与体验
- [dsh-paperpanel](https://github.com/Floatingfur/dsh-paperpanel) — Papers and data, unified inside DSH.  
  标签：dsh-plugin / 界面与体验
...
- [dsh-model-channel-manager](https://github.com/ArcaneOrion/dsh-model-channel-manager) — DSH model channel manager: roundrobin failover engine + model config panel  
  标签：dsh-plugin / 界面与体验
- [dsh-ticktick](https://github.com/PerryLink/dsh-ticktick) — TickTick (Dida365) task bridge for DeepSeek Harness: Session-header task panel (list filter, undone/completed views, search, drag reorder), 11 curated agent tools, settings card, over the official TickTick MCP endpoint.  
  标签：dsh-plugin / 界面与体验
...
- [dsh-internet-angel-desktop](https://github.com/Small-tailqwq/dsh-internet-angel-desktop) — 一款基于主播女孩重度依赖创建的 适用于 DSH 的 实验性主题皮肤 / Unofficial pixel desktop skin for DSH Web inspired by NEEDY GIRL OVERDOSE. Experimental, non-commercial fan project.  
  标签：dsh-plugin / 界面与体验
- [dsh-rgb-input-glow](https://github.com/zhangyuzhangyu233/dsh-rgb-input-glow) — DeepSeek Harness Web 插件：「光污染」RGB 流光皮肤（输入框/侧栏/hero 等 WebGL bloom + CSS 光效，独立皮肤不修改任何原皮肤）  
  标签：dsh-plugin / 界面与体验
- [dsh-skill-browser](https://github.com/loyalchiiina/dsh-skill-browser) — DSH skill library browser with floating ball panel, categories, Chinese descriptions, and an automatic skill-failure ledger (tools/result based). DSH 技能浏览器：悬浮球面板 + 分类 + 中文简介 + 技能失效台账自动登记  
  标签：dsh-plugin / 界面与体验
- [dsh-font-customizer](https://github.com/daha1216/dsh-font-customizer) — DeepSeek Harness（DSH）字体定制插件：更换界面/代码字体与正文字号，识别本机已安装字体，设置持久化保存。  
  标签：dsh-plugin / 界面与体验
...
- [dsh-ledger-compact](https://github.com/telagod/dsh-ledger-compact) — dsh-plugin: OMP-style ingress shaping plus optional local /fast-compact for DeepSeek Harness  
  标签：dsh-plugin / 界面与体验
- [DeepseekHarness-](https://github.com/Mknecar/DeepseekHarness-) — 本项目是一键静默启动DeepSeek Harness Web 的 Windows 桌面快捷方式包。这个快捷方式有鲸鱼图标并且不会出现黑色终端窗口。双击桌面上的DeepSeek快捷方式即可后台启动dsh web，并自动打开浏览器。也有一些简单纠错，如找不到 `dsh` / `npx` 时会弹出中文提示，不会说是点了没反应，还有一个，就是此脚本按自身所在目录解析路径，放到任意位置都能用  
  标签：dsh-plugin / 界面与体验
- [dsh-plugin-toolkit](https://github.com/kolawong/dsh-plugin-toolkit) — DeepSeek Harness personal toolkit: runtime-toggleable quality-of-life optimizations, each shipped as one card in the Toolkit settings page.  
  标签：dsh-plugin / 界面与体验
- [dsh-workspace-kit](https://github.com/ice5kysl/dsh-workspace-kit) — dsh (DeepSeek Harness) workspace kit: ⌘K Spotlight search, soft workspace archive/restore, per-workspace icons/colors, enhanced sidebar with drag reorder — Cordis bundle plugin (host tools + browser client).  
  标签：dsh-plugin / 界面与体验
- [chat-pace](https://github.com/TheGoodMorty/chat-pace) — Configurable chat auto-scroll for the DeepSeek Harness web UI  
  标签：dsh-plugin / 界面与体验
- [dsh-recovery-center](https://github.com/supery0ung/dsh-recovery-center) — Local plugin recovery points and an independent emergency recovery UI for DeepSeek Harness. English / 中文.  
  标签：dsh-plugin / 界面与体验
- [dsh-product-preview](https://github.com/choco9527/dsh-product-preview) — DSH插件 产物页面 可按照访达分栏的形式显示对话节点中的产物  
  标签：dsh-plugin / 界面与体验
- [dsh-eval-infra](https://github.com/TT-Wang/dsh-eval-infra) — Paired A/B evaluation infrastructure for DeepSeek Harness (dsh) components: interleaved repeated runs through the real runtime, verifier self-checks, one-variable arms, regression gating, cache- and calendar-aware cost, CLI + web UI  
  标签：dsh-plugin / 界面与体验
...
- [dsh-widechat](https://github.com/FraYoshi/dsh-widechat) — Widens the chat view of Deepseek Harness, so the text can fill you the entire screen. It also reduces max height of the text box of the user, plus other perks.  
  标签：dsh-plugin / 界面与体验
- [dsh-chat-fold](https://github.com/tntcannon5000/dsh-chat-fold) — Compact turn folding for DeepSeek Harness Web: restores collapsed completed turns in long sessions where stock folding stays disabled.  
  标签：dsh-plugin / 界面与体验
- [dsh-plugin-live-terminal](https://github.com/nguyenduclong-ict/dsh-plugin-live-terminal) — Real-time live streaming terminal output viewer for DeepSeek Harness (DSH Desktop)  
  标签：dsh-plugin / 界面与体验
- [dsh-oil-account](https://github.com/Dongfang81/dsh-oil-account) — A film-ready oil car vs EV ownership cost demo for DSH Better Sidebar.  
  标签：dsh-plugin / 界面与体验
- [dsh-pi-markdown](https://github.com/gunduziba/dsh-pi-markdown) — Pi TUI 终端风格 Markdown 样式插件，专为 DeepSeek Harness Web 设计  
  标签：dsh-plugin / 界面与体验
...
- [dsh-turn-outline](https://github.com/Andor-Z/dsh-turn-outline) — DSH 轮次轨迹侧边栏插件：按用户轮次折叠会话（输入+工具步骤+输出），一键跳回对话原位；零 AI、只读 | Turn-outline tab for dsh-better-sidebar: fold sessions by user turns with one-click jump-back; zero-LLM, read-only  
  标签：dsh-plugin / 界面与体验
- [dsh-client-ui-thinking-stats](https://github.com/Six6stRINgs/dsh-client-ui-thinking-stats) — 在底部Dock和每次对话尾部添加模型思考token统计的轻量化插件。A lightweight plugin that adds model thinking token statistics to the bottom Dock and the end of each conversation.  
  标签：dsh-plugin / 界面与体验
- [dsh-web-search-anysearch](https://github.com/VinciBeans/dsh-web-search-anysearch) — AnySearch-backed WebSearchProvider for DeepSeek Harness (dsh): switch web_search between the official DeepSeek search endpoint and AnySearch from a Plugins settings card.  
  标签：dsh-plugin / 界面与体验
- [dsh-universal-palette](https://github.com/yunmin311/dsh-universal-palette) — Dense translucent-glass Universal Palette for DeepSeek Harness Web — federates Commands, Sessions, Models, Conversation Hits with rich secondary actions and deterministic context/frecency ranking.  
  标签：dsh-plugin / 界面与体验
...
- [dsh-plugin-file-explorer](https://github.com/supanexus/dsh-plugin-file-explorer) — Workspace file tree and multi-tab editor in DeepSeek Harness — browse, edit, and preview files without leaving the chat UI.  
  标签：dsh-plugin / 界面与体验
- [dsh-md-picker](https://github.com/shangfr/dsh-md-picker) — DeepSeek Harness Web GUI 插件：在会话输入框左侧添加一个「文档 + Markdown」按钮，通过系统文件选择器添加附件并转换成 Markdown 格式。  
  标签：dsh-plugin / 界面与体验
- [see_the_screen](https://github.com/jaychouu/see_the_screen) — Windows desktop automation plugin for DeepSeek Harness (DSH) agents. Part of DSH Plugin Ecosystem.  
  标签：dsh-plugin / 界面与体验
- [dsh-prompt-enhance](https://github.com/DAIZHISEN/dsh-prompt-enhance) — A DSH web plugin: a star button that rewrites drafts into clearer prompts, combining PromptForge rule diagnosis with your session's default model. Bilingual (English / Chinese).  
  标签：dsh-plugin / 界面与体验
- [dsh-markdown-preview](https://github.com/thomasly/dsh-markdown-preview) — DSH web plugin: live Markdown + KaTeX math preview above the composer (official slot, zero core patches)  
  标签：dsh-plugin / 界面与体验
...
- [deepseek-whale-wallpaper](https://github.com/HeShen-1/deepseek-whale-wallpaper) — 🐋 Live dot-matrix whale wallpaper plugin for the DeepSeek Harness (dsh) web UI — 1,750 breathing WebGL2 particles, pointer vortices, light/dark themes · 深度求索 Harness 点阵鲸鱼动态壁纸插件  
  标签：dsh-plugin / 界面与体验
- [dsh-reasoning-bridge](https://github.com/shangdi178/dsh-reasoning-bridge) — Reasoning Bridge for dsh: hand high-cost reasoning to web AI (ChatGPT first, multi-target); keep local evidence, adoption gate, edits, and tests. Contract-based handoffs, versioned consent, SHA-256 receipts. Zero dependencies.  
  标签：dsh-plugin / 界面与体验
- [dshtools-sensevoice-input](https://github.com/ilovedyou6666-hub/dshtools-sensevoice-input) — 基于 SenseVoiceSmall（iic/SenseVoiceSmall）多语言语音理解模型的 DSH Desktop 本地语音输入插件。  
  标签：dsh-plugin / 界面与体验
- [dsh-theme-nier-automata](https://github.com/marcemira/dsh-theme-nier-automata) — NieR:Automata-inspired YoRHa theme for DeepSeek Harness web  
  标签：dsh-plugin / 界面与体验
- [dsh-token-rush](https://github.com/mikijiyun/dsh-token-rush) — DSH Web GUI 悬浮仪表：实时 token 消耗 / 估算金额 / 账户余额，高峰时段自动变橙红色。A floating token / cost / balance HUD that turns orange-red during DeepSeek rush hours.  
  标签：dsh-plugin / 界面与体验
- [dsh-plugin-preface-context](https://github.com/HuanLinOTO/dsh-plugin-preface-context) — 在每次会话开头固定注入一段用户配置的文本上下文（设置页输入框可编辑），作为模型可见的 instructions 注入第一轮请求。 | Injects a user-configured text block as model-visible instructions context at the start of every DSH session (editable from the settings page).  
  标签：dsh-plugin / 界面与体验
- [dsh-plugin-android-use](https://github.com/HuanLinOTO/dsh-plugin-android-use) — 让模型通过 adb 操作安卓手机的 DSH 插件（截图、UI 树、点击、滑动、输入文本、按键、打开应用） | DSH plugin exposing adb-based tools that let the model operate an Android phone (screenshot, UI dump, tap, swipe, input text, press keys, open apps).  
  标签：dsh-plugin / 界面与体验
- [dsh-plugin-better-plan](https://github.com/HuanLinOTO/dsh-plugin-better-plan) — better-plan: DSH 计划/待办侧边栏插件 | DSH plan/todo sidebar plugin  
  标签：dsh-plugin / 界面与体验
- [dsh-humanizer](https://github.com/liuwenji007/dsh-humanizer) — DeepSeek Harness 插件：生成前注入「人味」风格约束 + AI 味检测报告（纯代码、零 token、结果可复现）。dsh plugin: human-flavor style injection + AI-flavor detection report, judged by code.  
  标签：dsh-plugin / 界面与体验
- [dsh-more-message-actions](https://github.com/suzuran520yyz/dsh-more-message-actions) — DSH Web GUI 插件，用于对会话消息进行编辑、重试、高级重试和删除等功能的实现  
  标签：dsh-plugin / 界面与体验
...
- [dsh-log-viewer](https://github.com/CodeDice1024/dsh-log-viewer) — DSH 日志分析插件：Java 服务日志分析，AI 工具与 Web 仪表盘  
  标签：dsh-plugin / 界面与体验
- [dsh-plugin-voice-input-demo](https://github.com/Dee3526/dsh-plugin-voice-input-demo) — Minimal Chinese voice input plugin for the DeepSeek Harness Web UI  
  标签：dsh-plugin / 界面与体验
- [dsh-launcher](https://github.com/yyh-001/dsh-launcher) — DeepSeek Harness 轻量 Windows 启动器。选一个版本，启动 dsh web。  
  标签：dsh-plugin / 界面与体验
- [weibo-wallpaper-dsh-plugin](https://github.com/Zhiye-Tang/weibo-wallpaper-dsh-plugin) — 把微博博主的相册变成(DSH)Web 界面壁纸的宿主插件。每次打开 DSH 自动检查微博:同一天跳过,跨天/跨月增量下载,按月归档到本地 YYYY-MM 文件夹，首次启用自动回填今年1月到上个月的历史相册。即一个插件、一个你本人的微博 Cookie,就可同时跟踪多位博主(每人独立文件夹与状态) 三种显示模式 + 月份切换,控制条常驻页面底部。A DSH host plugin that syncs a Weibo blogger's album by month into local folders and shows them as the wallpaper behind  the DSH web UI. Cookie is required and stay local.  
  标签：dsh-plugin / 界面与体验
- [dsh-git](https://github.com/xia-sc/dsh-git) — DeepSeek Harness Web GUI 的完整 Git 管理插件：分支切换/fetch/pull/commit/push，可折叠悬浮面板 + 输入框胶囊  
  标签：dsh-plugin / 界面与体验
- [dsh-panda-calendar](https://github.com/runcat-tommy/dsh-panda-calendar) — 熊猫日历 · DSH client-plugin：公历/农历/干支/生肖/节气、传统与外国节日、中国法定节假日（含调休）、多城市天气 — token-free Chinese calendar & weather plugin for DeepSeek Harness Web.  
  标签：dsh-plugin / 界面与体验
- [dsh-web-verify-panel](https://github.com/EChan684/dsh-web-verify-panel) — 让 agent 的网页打开请求在 DSH 侧边栏内完成，不用弹出系统浏览器窗口  
  标签：dsh-plugin / 界面与体验
...
- [dsh-wsl-launcher](https://github.com/rqzhao1/dsh-wsl-launcher) — Windows desktop launcher for DeepSeek Harness Web running in WSL  
  标签：dsh-plugin / 界面与体验
- [dsh-stock-quote](https://github.com/gbeta/dsh-stock-quote) — DSH web plugin: collapsible draggable A-share quote widget (沪深300 + individual stocks, live price + change %)  
  标签：dsh-plugin / 界面与体验
...
- [foreman-dsh](https://github.com/AllDurham/foreman-dsh) — 贵脑+贱手: cloud-orchestrator + local-worker lanes (delegate & read-only scout) for DeepSeek Harness | 社区 Agent 预设  
  标签：dsh-plugin / Agent 与自动化
- [dsh-subagent-claude-live](https://github.com/Plaaasma/dsh-subagent-claude-live) — Claude Code as a real, streaming DeepSeek Harness subagent: session-backed child, per-call model/effort choice, live model list from the CLI  
  标签：dsh-plugin / Agent 与自动化
...
- [dsh-atom-market](https://github.com/ZiFan1117/dsh-atom-market) — DeepSeek Harness 社区插件（dsh-plugin）：把 Software Atom Market 变成 Agent 可逛/可校验/可投稿的能力库。Tools: atom_search/atom_read/atom_validate/atom_draft. 默认数据源=GitHub 商店，零配置。  
  标签：dsh-plugin / Agent 与自动化
- [dsh-zcode-persona](https://github.com/flashyiyi/dsh-zcode-persona) — Make the model believe it is inside the Z.ai Code (Zhipu ZCode) agent environment. DSH system-prompt waterfall plugin.  
  标签：dsh-plugin / Agent 与自动化
...
- [dsh-cron](https://github.com/GooDAnDReaDY/dsh-cron) — Scheduled cron tasks, background automation and agent execution for DeepSeek Harness.  
  标签：dsh-plugin / Agent 与自动化
- [dsh_HUDV1.0.0](https://github.com/Mknecar/dsh_HUDV1.0.0) — 做了一个DeepSeek Harness 的右上角悬浮任务台，它能够实时显示当前时段是波峰（贵/红）还是波谷（便宜/绿），还提供了模型、工具、Agent、定价与 24 小时的峰谷轨迹。整体设计风格就是左侧信息 + 右侧看板娘，希望大家喜欢。。  
  标签：dsh-plugin / Agent 与自动化
...
- [dsh-file-share](https://github.com/weibaohui/dsh-file-share) — dsh 插件 · 目录共享：任意可配置目录经 HTTP 在线浏览/上传/下载 + 对话框 @ 文件给 agent 处理  
  标签：dsh-plugin / Agent 与自动化
- [dsh-marketing-toolkit](https://github.com/1wanganshi/dsh-marketing-toolkit) — DSH 营销工具箱插件：侧边栏一键进入封面图生成 / 朋友圈文案 / 短视频脚本 / 开头黄金3秒检验四个营销 Agent 通道  
  标签：dsh-plugin / Agent 与自动化
- [routinekit](https://github.com/sjh9714/routinekit) — Turn successful agent tool calls into reviewed, parameterized routines you can replay. DSH, MCP, and native WebMCP.  
  标签：dsh-plugin / Agent 与自动化
- [dsh-kb](https://github.com/weibaohui/dsh-kb) — dsh 插件 · 团队知识库：离线知识共享（FDE 盒子），浏览/全文检索/加工入口；写路径收口 agent，基于 Karpathy LLM Wiki 模式  
  标签：dsh-plugin / Agent 与自动化
- [dsh-runtime-observability](https://github.com/tianjiqx/dsh-runtime-observability) — DeepSeek Harness 运行时可观测性插件。基于 OpenTelemetry 采集事件循环、内存、CPU、工作负载与子代理生命周期指标；支持断路器、ELU 降级联动及可选 Pyroscope 连续 Profiling。 Runtime metrics, continuous profiling & telemetry resilience plugin for DSH. Collects event loop, memory, CPU, workload & subagent lifecycle metrics; circuit breaker, ELU gating, optional Pyroscope profiling.  
  标签：dsh-plugin / Agent 与自动化
- [dsh-subagent-steer](https://github.com/Momojie-S/dsh-subagent-steer) — DSH plugin: steer_subagent - insert a new instruction into a running background subagent's current turn (step-boundary steering) or hard-restart its turn; send_message queueing stays the default  
  标签：dsh-plugin / Agent 与自动化
- [dsh-xiaoshuang](https://github.com/daixin315/dsh-xiaoshuang) — 小双 for DeepSeek Harness — 六层记忆人格插件 + 浮动视频形象（跟随 agent 状态表演 + TTS）  
  标签：dsh-plugin / Agent 与自动化
- [finance-agent-kit](https://github.com/davidalmeida90/finance-agent-kit) — Equity valuation skills and data MCPs for DeepSeek Harness and Claude Code. SEC filings, bottom-up sector beta, sourced implied ERP.  
  标签：dsh-plugin / Agent 与自动化
...
- [dsh-workbench-ecs](https://github.com/nishuoyang/dsh-workbench-ecs) — Alibaba Cloud Workbench CLI plugin — lets DeepSeek Harness's Agent directly control remote ECS instances.  
  标签：dsh-plugin / Agent 与自动化
- [dsh-plugin-task-runner](https://github.com/weixshaw/dsh-plugin-task-runner) — Task Runner 任务拆解模式：主代理拆解任务、子代理（默认本地模型）并行执行再综合结果，并发/模型可按机器配置。Decompose-and-dispatch agent preset for DeepSeek Harness.  
  标签：dsh-plugin / Agent 与自动化
...
- [dsh-better-model-select](https://github.com/sxy-kumako/dsh-better-model-select) — DSH 插件：模型列表跟随 provider API 实时更新 + 各 provider 用量徽标 (GLM/MiniMax coding plan, DeepSeek/OpenRouter/Kimi balance)  
  标签：dsh-plugin / Agent 与自动化
- [tencent-agent-mail-dsh-plugin](https://github.com/hun1315/tencent-agent-mail-dsh-plugin) — Tencent Agent Mail (agent.qq.com) workbench plugin for DeepSeek Harness: polling, LLM classification, fullscreen workbench, archive/trash, attachment download, mail-to-agent command channel. 腾讯Agent邮箱DSH插件  
  标签：dsh-plugin / Agent 与自动化
- [dsh-subagent-pi-plus](https://github.com/february2015/dsh-subagent-pi-plus) — Pi true-gateway plugin for DeepSeek Harness (DSH): /pi-lock direct connection to a durable Pi session — queued/steered conversation, live output, durable binding + auto-reattach. Personal fork of @deepseek-ai/dsh-subagent-codex.  
  标签：dsh-plugin / Agent 与自动化
...
- [dsh-plastic-memory](https://github.com/KKL08/dsh-plastic-memory) — Memory plugin with built-in curation and governance for DeepSeek Harness: 为你的 DSH 赋予可塑性记忆  
  标签：dsh-plugin / 记忆与知识
- [dsh-plugin-context-trim](https://github.com/Pasumao/dsh-plugin-context-trim) — 会话注入门控：按会话裁剪注入模型的 skill / tool / 提示词段落 · Per-session injection gate for DeepSeek Harness (dsh)  
  标签：dsh-plugin / 记忆与知识
- [dsh-prompt-refiner](https://github.com/glh5835/dsh-prompt-refiner) — DeepSeek Harness 提示词精炼插件：结构化改写 + 成本路由 + 缓存 + token 统计  
  标签：dsh-plugin / 记忆与知识
- [dsh-print3d](https://github.com/weisofns/dsh-print3d) — 3D printing & additive manufacturing plugin for DeepSeek Harness — parametric STL, calibration G-code, STL/G-code analysis, toolpath rendering, PrusaSlicer bridge, 5 skills, and a concise persona optimized for local models./面向 DeepSeek Harness 的 3D 打印与增材制造插件：参数化建模、校准 G-code、STL/G-code 分析、刀路可视化、PrusaSlicer 桥接，附 5 个技能与面向本地小模型的精简 persona。  
  标签：dsh-plugin / 记忆与知识
- [dsh-workspace-prompts](https://github.com/Fayelin12/dsh-workspace-prompts) — Per-workspace custom system prompts for DeepSeek Harness (DSH): one active prompt per workspace, captured into a session at creation time, editable from a session-header dialog.  
  标签：dsh-plugin / 记忆与知识
- [dsh-script-manager](https://github.com/kaixinguo360/dsh-script-manager) — DSH 自定义操作脚本管理插件，将验证过的操作流程固化为可复用脚本，不耗 token、支持参数化和执行验收。  
  标签：dsh-plugin / 记忆与知识
- [dsh-plugins](https://github.com/wuxiangru915/dsh-plugins) — DeepSeek Harness plugins by wuxiangru915: session manager, review loop, session navigator  
  标签：dsh-plugin / 记忆与知识
...
- [dsh-memgas](https://github.com/quqxui/dsh-memgas) — DeepSeek Harness 长期记忆插件：四通道检索融合 + 记忆演化闭环，记忆构建与检索方法来自 ICLR 2026 论文 MemGAS。Long-term memory plugin for DeepSeek Harness (dsh).  
  标签：dsh-plugin / 记忆与知识
- [dsh-mattpocock-skills](https://github.com/addie-ace/dsh-mattpocock-skills) — DeepSeek Harness 技能包插件（汉化版）：mattpocock/skills 25 个 promoted 技能全文汉化打包，经 ctx.skills 注册。原出处 https://github.com/mattpocock/skills（Matt Pocock，MIT）。  
  标签：dsh-plugin / 记忆与知识
...
- [dsh-context-maid](https://github.com/Icstick/dsh-context-maid) — DeepSeek Harness 自动上下文策展插件：tool 输出内容感知瘦身 + 无效日志清理 + 工作流/记忆钉扎保护 + 先归档后压缩 + 摘要模型可配  
  标签：dsh-plugin / 记忆与知识
- [relay-dsh-plugin-skill-creator](https://github.com/yangbobo2021/relay-dsh-plugin-skill-creator) — DSH plugin that turns completed conversations into reusable, validated Skill bundles  
  标签：dsh-plugin / 记忆与知识
- [dsh-github-mcp](https://github.com/ZIye1208/dsh-github-mcp) — DSH 插件：GitHub MCP 连接插件，token 存 DSH 凭据中心(.credentials.yaml)，自动捆绑安装面板插件 dsh-github-mcp-hint（可独立卸载）。  
  标签：dsh-plugin / 记忆与知识
- [dsh-external-session](https://github.com/XiaoYuOvO/dsh-external-session) — Portable Git-backed external session persistence plugin for DeepSeek Harness  
  标签：dsh-plugin / 记忆与知识
- [dsh-session-sweeper](https://github.com/HrxSpace/dsh-session-sweeper) — DSH 插件，扫描管理电脑本地所有终端的会话（Claude Code、Codex、Workbuddy、DSH等）  
  标签：dsh-plugin / 记忆与知识
- [dsh-idle-compactor](https://github.com/QuanhuZeYu/dsh-idle-compactor) — Idle-triggered context compaction plugin for DeepSeek Harness: compact a session once it goes quiet past a configurable token floor.  
  标签：dsh-plugin / 记忆与知识
- [dsh-sess](https://github.com/youridol/dsh-sess) — Session manager for DeepSeek Harness (dsh-v0.1.2-alpha.5): permanently delete cold sessions and manage archived sessions via the official Profile Bundle / Cordis APIs. MIT.  
  标签：dsh-plugin / 记忆与知识
- [dsh-conversation-atlas](https://github.com/anlew07/dsh-conversation-atlas) — Project-level conversation memory explorer for DeepSeek Harness with related memories, cross-session bridges, BM25 search, and exact chat navigation.  
  标签：dsh-plugin / 记忆与知识
- [prompt-optimizer-plugin](https://github.com/naitoupi/prompt-optimizer-plugin) — DeepSeek Harness (DSH) plugin: ? optimize the composer draft with the current model. Installable bundle (dsh.bundle + dsh.client); Settings tab: on/off switch, generation params, editable system prompt with reset-to-default.  
  标签：dsh-plugin / 记忆与知识
- [dsh-session-spend](https://github.com/weiwang988/dsh-session-spend) — DSH 客户端插件：实时会话花费（¥）· 官方峰谷计价 + 节省分解 DSH client plugin: live session cost readout (¥) with official DeepSeek peak/valley pricing and savings breakdown  
  标签：dsh-plugin / 记忆与知识
- [conceptnet-dsh-plugin](https://github.com/wushu75/conceptnet-dsh-plugin) — ConceptNet intent classification plugin for DeepSeek Harness. Classifies enterprise voice and text commands into 4 execution layers — Basic, Context-Aware, Predictive, Autonomous — at 100% accuracy. 9 languages. Token-free. No LLM required.  
  标签：dsh-plugin / 记忆与知识
- [kimi-ppt-skill](https://github.com/OMSociety/kimi-ppt-skill) — DSH (DeepSeek Harness) presentation skill: PPT/演示文稿生成与导出，含 DSH 内纯本地导出 (python-pptx) 与本地预览渲染 (Pillow)。  
  标签：dsh-plugin / 记忆与知识
- [dsh-plugin-voice-input](https://github.com/porcelaintech/dsh-plugin-voice-input) — Voice input plugin for DeepSeek Harness: a minimal mic control in the composer with continuous bilingual recognition, context-aware term correction, punctuation, and a live waveform  
  标签：dsh-plugin / 记忆与知识
- [dsh-status-hub](https://github.com/HUSTforever/dsh-status-hub) — Bilingual DeepSeek Harness status hub for sessions, MCP, skills, plugins, and connection health  
  标签：dsh-plugin / 记忆与知识
- [dsh-opencode-free-models](https://github.com/yu-wenchao/dsh-opencode-free-models) — 无限免费额度的deepseek harnes 免费模型插件-免费api-DeepSeek Harness (DSH) 插件 — 在聊天界面里实时展示 OpenCode Zen 的最新免费模型，无需登录、无需密钥，直接在模型选择器选用即可对话。切换网络实现无限额度，无限量token自由！  
  标签：dsh-plugin / 记忆与知识
- [dsh-fullwidth-punctuation](https://github.com/addie-ace/dsh-fullwidth-punctuation) — DeepSeek Harness 插件：让 AI 回答输出中文全角标点（用户直接复制回答内容也是全角）。监听 llm/stream 做智能半角→全角转换，跳过代码块/数字/URL/markdown，并注入 system prompt 提示。  
  标签：dsh-plugin / 记忆与知识
- [dsh-side-session](https://github.com/123dbl/dsh-side-session) — DSH side-chat (旁会话) plugin: launch a fresh continuable parallel session from the conversation header while the main session keeps running.  
  标签：dsh-plugin / 记忆与知识
...
- [dsh-chem](https://github.com/TYEclipse/dsh-chem) — Deterministic chemistry math for DeepSeek Harness: formula parsing + molar mass, mass/mole/particle conversions, solution concentration and pH math — zero runtime dependencies  
  标签：dsh-plugin / 工具与连接
- [dsh-search-model](https://github.com/lovstudio/dsh-search-model) — Search-enabled composer model seat for DeepSeek Harness (sticky fuzzy search)  
  标签：dsh-plugin / 工具与连接
...
- [dsh-extras](https://github.com/CatheadOwl/dsh-extras) — Out-of-tree plugin suite for deepseek-harness (dsh): markdown tooling, relates tables, review-evals dispatch.  
  标签：dsh-plugin / 工具与连接
- [dsh-xmap](https://github.com/lvguqingyou/dsh-xmap) — xmap Excel schema-map tools for DeepSeek Harness (7 tools: sheets/headers/hash/extract/verify/save-spec/aggregate)  
  标签：dsh-plugin / 工具与连接
- [dsh-rs](https://github.com/Saidoua/dsh-rs) — In-process grep and glob for the DeepSeek Harness search tools, on ripgrep's library crates — a napi-rs addon, measured in the harness against the packaged ripgrep spawn  
  标签：dsh-plugin / 工具与连接
- [dsh-reelsmaker](https://github.com/aayan-cloud/dsh-reelsmaker) — DeepSeek Harness plugin: turn narration lines into a finished vertical reel. Free neural voice, burned-in captions, no API keys.  
  标签：dsh-plugin / 工具与连接
- [dsh-tool-zg](https://github.com/gunduziba/dsh-tool-zg) — DeepSeek Harness 原生 zvec-grep (zg) 代码语义搜索工具插件，支持按预设动态过滤  
  标签：dsh-plugin / 工具与连接
- [dsh-reply-language](https://github.com/CAI-MH/dsh-reply-language) — Force the DeepSeek Harness model to reply in a chosen language (default Simplified Chinese), switchable anytime via tools. 强制大模型使用指定语言回复。  
  标签：dsh-plugin / 工具与连接
...
- [dsh-browser-use](https://github.com/helloproblems/dsh-browser-use) — 为 DeepSeek Harness (DSH) 插件生态打造的浏览器自动化插件  
  标签：dsh-plugin / 工具与连接
- [dsh-webhook-notifier](https://github.com/masknull/dsh-webhook-notifier) — DSH 事件通知插件：AI 回合完成、权限申请、提问、出错、开始运行等 5 种场景发生时，向 Webhook 发送自定义 HTTP 通知，支持 GET/POST、预设字段/自定义文本、type+type_cn、发送日志  
  标签：dsh-plugin / 工具与连接
- [dsh-mcp-market](https://github.com/LeonxLJX/dsh-mcp-market) — The front door to MCP for DeepSeek Harness. Search a verified catalog of MCP servers and get the exact cordis.yml row that wires one into the official @deepseek-ai/dsh-mcp-client bridge.  
  标签：dsh-plugin / 工具与连接
...
- [dsh-visualizer-widget](https://github.com/lovezi0/dsh-visualizer-widget) — DeepSeek Harness 的会话流「内联可视化」插件：模型产出 SVG / HTML 源码后，交给浏览器渲染成一张可交互的内联卡片，源码只进卡片、不回灌模型上下文。  
  标签：dsh-plugin / 多模态与视觉
- [dsh-btw](https://github.com/WLV-ZEDD/dsh-btw) — DeepSeek Harness Side-Assistant Dock & Drawer  
  标签：dsh-plugin / 多模态与视觉
- [dsh-document-drop](https://github.com/Su2uka111/dsh-document-drop) — Document drag-and-drop plugin for DeepSeek Harness: PDF/Word/Excel/text auto-parsed with dual-track context (inline + doc_read), native WinRT OCR for scans. Zero upstream changes, hot-injectable. | DeepSeek Harness 文档拖拽读取插件：PDF/Word/Excel/文本自动解析，双轨上下文（内联 + doc_read 按需检索），扫描件原生 WinRT OCR。零修改上游，运行时热注入。  
  标签：dsh-plugin / 多模态与视觉
...
- [dsh-opencode-models](https://github.com/Retr67/dsh-opencode-models) — DeepSeek Harness (DSH) plugin: one-click OpenCode Go model sync in the settings panel - fetch all server models, probe availability & vision, write back to llm-pi-ai config  
  标签：dsh-plugin / 多模态与视觉
- [dsh-plugin-product-design](https://github.com/Kayungko/dsh-plugin-product-design) — Product Design workflow suite for DeepSeek Harness (DSH Desktop): a router plus nine pd-* skills — minimum brief gate, three visual directions, evidence-grounded audits/research, URL cloning, frontend builds with a design-QA gate. Original rewrite of the workflow methodology.  
  标签：dsh-plugin / 多模态与视觉
- [dsh-plugin-chat-enhance](https://github.com/supanexus/dsh-plugin-chat-enhance) — Searchable model picker with recent chips, vision capability tags, and composer image upload for DeepSeek Harness.  
  标签：dsh-plugin / 多模态与视觉
- [dsh-plan-bridge](https://github.com/TaylorSwitiger/dsh-plan-bridge) — DeepSeek Harness (dsh) plugin: one local bridge for ChatGPT Codex / Claude / ZCode (GLM) / Qwen subscription plans — settings status card, one-click provisioning, zero core changes. npm: @taylorswitiger/dsh-plan-bridge  
  标签：dsh-plugin / 多模态与视觉
- [dsh-raw-html-v2](https://github.com/plolpl789/dsh-raw-html-v2) — VCP visual-synesthesia plugin v2 for DeepSeek Harness (official Slot API): raw-HTML vcp-root cards, streaming render, KaTeX/Mermaid/SVG, builtin fonts  
  标签：dsh-plugin / 多模态与视觉
- [dsh-vision-pro-bridge](https://github.com/ShaineDemo/dsh-vision-pro-bridge) — Give text-only DeepSeek-V4-Pro vision — zero new dependencies, DeepSeek-only routing (images described by deepseek-v4-flash-vision-exp, then answered by V4-Pro).  
  标签：dsh-plugin / 多模态与视觉
- [dsh-plugin-vision](https://github.com/aijunjiang/dsh-plugin-vision) — Give your DSH agent eyes via any OpenAI-compatible vision model - 11 provider presets (Doubao/Qwen-VL/GLM-V/OpenAI/Gemini/Ollama...), capability checkboxes that inject live prompt guidance, and analysis of images the user drops into the chat; the agent writes its own observation prompt, and base64 never enters its context.  
  标签：dsh-plugin / 多模态与视觉
- [dsh-vision-patch](https://github.com/ryasrk/dsh-vision-patch) — Patch custom provider vision model  
  标签：dsh-plugin / 多模态与视觉
...
- [dsh-time-machine](https://github.com/GooDAnDReaDY/dsh-time-machine) — DSH 智能快照插件：工作区安全护栏与一键回滚。  
  标签：快照 / 回滚 / 安全
- [dsh-pr-guardian](https://github.com/Harzva/dsh-pr-guardian) — Authored PR feedback inbox with shared Codex/DSH progress and read-only GitHub collection.  
  标签：dsh-plugin / 安全与运维
...
- [dsh-clinebot](https://github.com/GooDAnDReaDY/dsh-clinebot) — DeepSeek Harness companion for ClineBot / ClinePass: dedicated settings page, live quota dashboard, in-UI key saving, curated & custom models, /cline command.  
  标签：dsh-plugin / 安全与运维
- [dsh-tool-args-unwrap](https://github.com/shy19805/dsh-tool-args-unwrap) — Fix malformed tool-call arguments (nested arguments wrappers, key aliases) at the dsh llm/stream seam. 修复畸形工具参数的 dsh 插件  
  标签：dsh-plugin / 安全与运维
...
- [dsh-security-guard](https://github.com/weisofns/dsh-security-guard) — DSH plugin security guard: 28-rule static scanner, risk scoring, whitelist/blacklist policy, local web dashboard and in-DSH risk popups./DSH 插件安全卫士：28 条规则静态扫描、风险评分、白名单/黑名单、本地 Web 仪表盘与 DSH 内风险弹窗。  
  标签：dsh-plugin / 安全与运维
- [dsh-reach](https://github.com/PerryLink/dsh-reach) — Multi-channel decision & remote-control bridge for DeepSeek Harness: pushes any workspace's approval/question cards to IM channels (WeChat iLink first) and answers them from chat, with a session console, per-channel security, and an open push service.  
  标签：dsh-plugin / 安全与运维
- [dsh-secure-context-fix](https://github.com/Li-Mingshuang/dsh-secure-context-fix) — Fix the DeepSeek Harness Web GUI over plain HTTP on a LAN: injects a crypto.randomUUID polyfill (getRandomValues-backed) before dsh bundle code runs  
  标签：dsh-plugin / 安全与运维
...
- [dsh-plugin-supanexus-core](https://github.com/supanexus/dsh-plugin-supanexus-core) — SupaNexus platform core: branding, OAuth quick setup, dual-line model provider, and app plugin market  
  标签：dsh-plugin / 安全与运维
- [dsh-research-writing](https://github.com/Flan246/dsh-research-writing) — Research writing bundle for DeepSeek Harness: literature search/citation + LaTeX/BibTeX guard in one install.  
  标签：dsh-plugin / 安全与运维
- [dsh-opencode-go-usage](https://github.com/mouseteamlucky/dsh-opencode-go-usage) — DSH (DeepSeek Harness) web plugin: floating widget showing real-time OpenCode Go subscription usage (rolling / weekly / monthly) for every API key. Maintained fork of @xiaweiliang060035/dsh-opencode-go-usage.  
  标签：dsh-plugin / 安全与运维
- [dsh-arch-advisor](https://github.com/LeeGuanWei-a/dsh-arch-advisor) — 给 DeepSeek Harness 装一位"读过架构书"的架构顾问：实时查阅 40 教程/31 模板/6 案例，引导设计并产出需求/概要/详细/数据库文档。· Persistent DSH plugin: an in-chat architecture consultant that queries the awesome-architecture knowledge base and guides engineering-doc authoring.  
  标签：dsh-plugin / 安全与运维
- [dsh-session-doctor](https://github.com/luoxin10086/dsh-session-doctor) — Session doctor for DeepSeek Harness: scan/repair/watch stored session logs against loader-mirror validation, plus render-contract audit. 会话体检与修复插件。  
  标签：dsh-plugin / 安全与运维
- [dsh-agy-safe](https://github.com/Jonah-Wu23/dsh-agy-safe) — DeepSeek Harness plugin: connect logged-in Antigravity CLI (agy) as chat and subagent model provider  
  标签：dsh-plugin / 安全与运维
- [dsh-disk-manager](https://github.com/wangzhanchao883/dsh-disk-manager) — DeepSeek Harness plugin: scan the whole C drive, classify every big item (safe cache / relocatable / junction-movable / abandoned / config-memory redline / personal), delete/move/redirect on demand with dry-run & undo. C?????????:????+??+?/?/??,????????  
  标签：dsh-plugin / 安全与运维
...
- [dsh-git-auth-plugin](https://github.com/Wisdoverse/dsh-git-auth-plugin) — GitHub and GitLab CLI authentication through DSH credentials or environment tokens, with workspace-local SSH deploy keys and plugin settings.  
  标签：dsh-plugin / 安全与运维
- [dsh-plugin-health](https://github.com/ice5kysl/dsh-plugin-health) — Zero-dependency health-check CLI for DeepSeek Harness (dsh) plugins: manifest/npm/repo/docs checks + read-only-surface security scan. 零依赖的 dsh 插件体检 CLI。  
  标签：dsh-plugin / 安全与运维
- [dsh-security-guard](https://github.com/ruanhaodong-tt/dsh-security-guard) — DSH runtime security guard plugin - loader import confinement, HTTP Host header validation (QVD-2026-52631/57410). AI-assisted.  
  标签：dsh-plugin / 安全与运维
- [dsh-tool-lipsync](https://github.com/yu-wenchao/dsh-tool-lipsync) — 免 Key、免登录的 AI 对口型口播视频生成插件，为 DeepSeek Harness (DSH) 注册 5 个工具，使用免费开放 API 生成对口型视频。FreeLipSync lip-sync video plugin for DeepSeek Harness - No API key required  
  标签：dsh-plugin / 安全与运维
- [dsh-we-wallpaper](https://github.com/leeseo39/dsh-we-wallpaper) — Wallpaper Engine assets as DeepSeek Harness dynamic background — upstream skin-center 0.3.14 source snapshot + self-built WE engine research  
  标签：dsh-plugin / 安全与运维
...
- [dsh-evolver](https://github.com/cofy-x/dsh-evolver) — Auditable, verifier-gated self-evolution for DeepSeek Harness.  
  标签：dsh-plugin / 安全与运维
- [dsh-file-preview](https://github.com/softspark/dsh-file-preview) — Read-only in-conversation file preview for DeepSeek Harness: a session-authorized host Remote and a sanitizing browser modal, installed as one profile bundle.  
  标签：dsh-plugin / 安全与运维
- [dsh-dsml-artifact-guard](https://github.com/GooDAnDReaDY/dsh-dsml-artifact-guard) — DeepSeek Harness DSML artifact sanitizer for leaked protocol tags  
  标签：dsh-plugin / 安全与运维
- [dsh-cloak](https://github.com/zhangmiao03/dsh-cloak) — Context firewall for DeepSeek Harness: credentials in tool results become opaque placeholders before they reach the model — the secret never enters context.  
  标签：dsh-plugin / 安全与运维
- [dsh-mobile-access-plugin](https://github.com/Mandarin715/dsh-mobile-access-plugin) — DSH 手机远程通道插件：一键生成并部署 frp 隧道 + Basic Auth 反代 + 开机自启（通用，不锁 DSH 发行版）  
  标签：dsh-plugin / 安全与运维
- [rein-scene-forge-dsh-plugin](https://github.com/peng2048/rein-scene-forge-dsh-plugin) — DeepSeek Harness plugin for Rein Scene Forge robot scene authoring  
  标签：dsh-plugin / 安全与运维
- [dsh-realtimeavatar](https://github.com/theinfluencecompany/dsh-realtimeavatar) — Realtime Avatar (realtimeavatar.ai) for the DeepSeek Harness (dsh): harness-held API key, public docs as skills, rta_* tools, /rta onboarding, zero runtime dependencies  
  标签：dsh-plugin / 安全与运维
- [dsh-model-account-login](https://github.com/hongbaiqi/dsh-model-account-login) — Persistent DSH model-account authorization UI for ChatGPT, Claude, and other llm-pi-ai login flows  
  标签：dsh-plugin / 安全与运维
- [dsh-recovery](https://github.com/AndyZHENG0715/dsh-recovery) — DSH self-recovery: zero-dependency CLI (scan/snapshot/rollback/safemode/boot-probe/doctor) + in-process watchdog plugin. Process-external boot gate, tiered recovery ladder, safe/repair profiles.  
  标签：dsh-plugin / 安全与运维
- [dsh-claim-guard](https://github.com/xuxucodepractice-code/dsh-claim-guard) — A deterministic pre-write claim guard for supported DeepSeek Harness file tools.  
  标签：dsh-plugin / 安全与运维
...
- [dsh-godot-game-studio](https://github.com/jeffxuan/dsh-godot-game-studio) — Seven bounded Godot 4 MCP tools for DeepSeek Harness: audit, test, simulate, validate, and release  
  标签：dsh-plugin / 安全与运维
- [dsh-mcp-oauth-client](https://github.com/fangzaozao/dsh-mcp-oauth-client) — Universal MCP manager and client bridge for DeepSeek Harness (DSH)  
  标签：dsh-plugin / 安全与运维
